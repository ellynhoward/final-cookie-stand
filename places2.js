...
document.getElementById('user_form').addEventListener('submit', function (event){ //fire function and pass event in//
  ...
  var eventMinCust = parseInt(event.target.min_customer.value);
  var eventMaxCust = parseInt(event.target.max_customer.value);
  var eventAvgCust = parseFloat(event.target.avg_customer.value);
  console.log(eventStoreLocation);
  console.log(eventMinCust);
  console.log(eventMaxCust);
  console.log(eventAvgCust);

  var newStore = new CookieShop (eventStoreLocation, eventMinCust, eventMaxCust, eventAvgCust);
